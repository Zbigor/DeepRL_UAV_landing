...
class HummingbirdLandBasic(hummingbird_env.HummingbirdEnv):
    def __init__(self):
        ...
        # steps limit for each episode - equivalent to  20 seconds, since
        # the actions are taken at the rate of 30 Hz
        self.step_limit = 600;
        ...
        self.approach_velocity_reward = 0.0
        ...
        super(HummingbirdLandBasic, self).__init__()
...
#        self.takeoff()
    ...
    def _is_done(self, observations):
        ...
        self.crashed = (contact == 1) and (self.average_velocity_at_contact(30) >= 1.2)
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            self.cumulated_flipping_reward += flipping_reward + self.drone_flipped*(-100)
            ...
            reward += shaping_reward + flipping_reward + approach_angle_reward + self.exiting_boundaries_reward + self.not_ending_point_reward
            ...
            self.safe_landing_reward = self.landed_safely * (self.safe_landing_points + 
                                                             ...
                                                             2.0 * np.exp(5 - np.sqrt(self.current_obs[0]**2 + self.current_obs[1]**2))) 
            ...
            self.approach_velocity_reward = self.approach_velocity_reward(self.average_velocity_at_contact(30))*(self.ground_contact) + self.crashed * (-100)
            rewards_msg.approach_velocity_rew = self.approach_velocity_reward
            
            self.early_stop_reward = (self.cumulated_steps<30) *(30-self.cumulated_steps)*(-500)
            rewards_msg.early_stop_rew = self.early_stop_reward
            
            self.goal_proximity_bonus = (not (self.landed_safely)) * 5* np.exp(5-(np.sqrt(self.current_obs[0]**2+
                                                                                 self.current_obs[1]**2 + 
                                                                                 self.current_obs[2]**2)))
            rewards_msg.goal_prox_bonus_rew = self.goal_proximity_bonus
            
            flipping_reward = (not self.drone_flipped) * (self.flip_reward(self.current_obs[3],self.prev_obs[3])*0.5 + 
                                                          self.flip_reward(self.current_obs[4],self.prev_obs[4])*0.5) + self.drone_flipped*(-1000)
            ...
            reward += self.safe_landing_reward + (self.exiting_boundaries_reward + self.approach_velocity_reward + self.early_stop_reward + shaping_reward +
                                                  self.goal_proximity_bonus + flipping_reward)
    ...
    def approach_velocity_reward(self,velocity):
        if velocity>1.6:
            return -100
        if (velocity<=1.6) and (velocity >=0.8):
            return - 2.0 * np.exp(2*(velocity-0.8))
        if velocity < 0.8:
            return +2.0 * np.exp(1*(0.8-velocity))
    ...
    def flip_reward(self,angle,prev_angle):
        if np.abs(angle) < 0.26:
            return 5*np.exp(1*(0.26-np.abs(angle)))
        if (np.abs(angle)<1.57)and(np.abs(angle)>=0.26):
            return -5.0*np.exp(2*(np.abs(angle)-0.26)) - 5*(np.abs(angle)>np.abs(prev_angle))
    ...
    def has_landed_safely(self,observations):
        ...
        workspace
        """
        if ((self.current_act[3] <= 0.00001) and (observations[2]<=0.075) and 
    ...
    def average_velocity_at_contact(self,time_frame):
        ...
        if length >(time_frame + 1): 
            return (self.height_log[-1] - self.height_log[-1-time_frame])/(time_frame * 1/30)
        else:
            return (self.height_log[-1] - self.height_log[0])/(length * 1/30)
