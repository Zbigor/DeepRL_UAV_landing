...
def ddpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    rospy.logwarn("obs dim = " + str(obs_dim))
    act_dim = env.action_space.shape[0]
    rospy.logwarn("act_dim = "+ str(act_dim))
...
if __name__ == '__main__':
    # instead of parsing the arguments, read them from parameter server
#    import argparse
...
#    args = parser.parse_args()
    ...
    env_land = rospy.get_param("/ddpg/env")
    ...
    logger_kwargs = setup_logger_kwargs(exp_name, my_seed)

    ddpg(lambda : gym.make(env_land), actor_critic=core.mlp_actor_critic,
         ac_kwargs=dict(hidden_sizes=[200,300,400]*1),
         gamma=my_gamma, seed=my_seed, epochs=my_epochs,
